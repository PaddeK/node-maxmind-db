...
function IPParser(ip) {
    if (ip.indexOf('.') !== -1) {
        if (ip.indexOf('::') !== -1){
            return IPParser.parseIPv4_Mapped_IPv6(ip)
...
IPParser.parseIPv4_Mapped_IPv6 = function parseIPv4_Mapped_IPv6(ip){
    return ipv4Buffer(IPAddress.parseIPv4_Mapped_IPv6(ip));
}
